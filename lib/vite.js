...
var AST = class _AST {
  ...
  toMMPattern() {
    ...
    const [re, body, hasMagic3, uflag] = this.toRegExpSource();
    const anyMagic = hasMagic3 || this.#hasMagic || this.#options.nocase && !this.#options.nocaseMagicOnly && glob2.toUpperCase() !== glob2.toLowerCase();
    if (!anyMagic) {
      return body;
    }
  ...
  static #parseGlob(glob2, hasMagic3, noEmpty = false) {
    let escaping = false;
    let re = "";
    let uflag = false;
    ...
    return [re, unescape(glob2), !!hasMagic3, uflag];
  }
};
...
var extractPath = (files, path3) => {
  files.forEach((file) => {
    const toto = file.replace(path3, "").split("/");
    const titi = toto.map((s) => s === "" ? "/" : s);
    console.log(titi);
  });
};
var createStack = (files) => {
...
var createImports = (files) => {
  return files.map((t, i) => `
import Screen${i} from '${path2.resolve(t)}';

`);
};
var loader = ({ path: pathOption }) => {
